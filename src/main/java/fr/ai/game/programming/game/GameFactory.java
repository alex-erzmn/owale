    ...
    public static Game createAwaleGame(GameMode gameMode) {
        switch (gameMode) {
            case PLAYER_VS_AI_LOCAL -> {
                ...
                AIPlayer player2 = new AIPlayer(new AIManagerAlex(board));
                return new Game(board, player1, player2);
            }
            case AI_VS_PLAYER_LOCAL -> {
                Board board = new Board();
                AIPlayer player1 = new AIPlayer(new AIManagerAlex(board));
            ...
            case AI_VS_AI_LOCAL -> {
                Board board = new Board();
                AIPlayer player1 = new AIPlayer(new AIManagerAlex(board));
                AIPlayer player2 = new AIPlayer(new AIManagerNolan(board));
                return new Game(board, player1, player2);
            }
